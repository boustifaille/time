...
def recolteH():
    ...
    debutMatin = horaires["matin"]["debut"]
    pauseMAtin = horaires["matin"]["pause"]
    finMatin = horaires["matin"]["fin"]

    debutAPmidi = horaires["apres-midi"]["debut"]
    ...
    heureFinDuCoursmatin = str(datetime.time(11,40,00))
    heureFinDuCoursapres = str(datetime.time(16,5,00))
    heureFinDuCourselecum = str(datetime.time(14,20,00))

    heureDebutM = str(datetime.time(9,45,00))
    heureDebutA = str(datetime.time(15,5,00))

    # formater proprement les heures
    recolteH.d1 = datetime.datetime.strptime(recolteH.heureActuelle, '%H:%M:%S')# maintenat
    recolteH.d2 = datetime.datetime.strptime(heureFinDuCoursmatin, '%H:%M:%S')    # fin matinée
    recolteH.d3 = datetime.datetime.strptime(heureFinDuCoursapres, '%H:%M:%S')    # fin après midi
    recolteH.d4 = datetime.datetime.strptime(heureFinDuCourselecum, '%H:%M:%S')    # fin d'elecnum

    recolteH.d5 = datetime.datetime.strptime(heureDebutM, '%H:%M:%S')    # Matin
    recolteH.d7 = datetime.datetime.strptime(heureDebutA, '%H:%M:%S')    # Après-midi
